    ...
    for i in range(len(segments_bins_prototype)):
        if len(ground_lines[i]) > 0:
            ...
            init_plot_2D(
                ...
                + str(i + 1)
                + " | Degrees: "
                + str(round((i * DELTA_ALPHA) * 180 / math.pi, 2))
                + " to "
                + str(round((i + 1) * DELTA_ALPHA * 180 / math.pi, 2))
                + " | Points: "
                + str(ground_lines[i][0][4]),
...
def plot_bad_boys(cluster, bad_boys, good_boys, segs_to_count):
    ax = init_plot_3D(
        str(cluster[0]) + " " + str(cluster[1]) + " stc: " + str(segs_to_count), "x", "y", "Height", 45, 45
    )
