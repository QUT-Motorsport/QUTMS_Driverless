    ...
    for i in range(len(point_cloud)):  # iterate through each point
        ...
        if point[Z] <= 100 and point[X] > 0:  # only take points in front 180 degrees
            ...
            if bin_idx != -1:  #
                ...
                segments_bins[seg_idx][bin_idx].append([point[X], point[Y], point[Z]])
...
def dist_points_3D(x_0: list, p_1: list, p_2: list) -> float:
    ...
    p_norm: float = math.sqrt((p_2[X] - p_1[X]) ** 2 + (p_2[Y] - p_1[Y]) ** 2 + (p_2[Z] - p_1[Z]) ** 2)
    # return distance
        ...
        for j in range(NUM_BINS):
            for k in range(len(segments_bins[i][j])):
                ...
                if point[2] < line_height + 0.10:  # Make this a constant
                    ...
                    dynamic_T_D_GROUND = (
                        2 * ((j + 1) * BIN_SIZE) * math.tan(DELTA_ALPHA / 2) + BIN_SIZE * 1.3
...
def object_reconstruction(cluster_centers, segments_bins, ground_lines) -> List[List]:
    ...
    for i in range(len(cluster_centers)):
        ...
        if (
            cluster[Z] >= line_height + (CONE_HEIGHT / 2) * 0.8 and cluster[Z] <= line_height + (CONE_HEIGHT / 2) * 1.2
...
def get_ground_plane(point_cloud: List[List]) -> List[List]:
    ...
    if VISUALISE:
        vis.plot_segments_bins(segments_bins, False)
    ...
    if VISUALISE:
        vis.plot_ground_lines_3D(segments_bins_prototype, ground_plane, False)
    ...
    if VISUALISE:
        vis.plot_labelled_points(labelled_points, ground_plane)
...
def lidar_init(_visualise: bool, _display: bool, _figures_dir: str, _max_range: int = 16):
    ...
    if math.pi % DELTA_ALPHA != 0:
        raise ValueError(
            "Value of DELTA_ALPHA:", DELTA_ALPHA, "does not divide a circle into an integer-number of segments."
        )
