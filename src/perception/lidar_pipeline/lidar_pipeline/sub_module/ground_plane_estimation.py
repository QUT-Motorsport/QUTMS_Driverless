        ...
        for j in range(len(segments[i])): 
            ...
            if point[2] < line_height + 0.08: # Make this a constant
                ...
                for k in range(1, num_lines):
                    ...
                    if (dist_to_line < closest_dist):
                        closest_dist = dist_to_line
                dynamic_T_D_GROUND = (BIN_SIZE*(bin_index+2))*math.tan(DELTA_ALPHA/2) # Solved for gradient of segment wrt points and distance
                if (closest_dist < T_D_MAX and closest_dist < dynamic_T_D_GROUND):
                    is_ground = True
            segments[i][j].append(is_ground)
    return segments
...
def main(point_cloud, _visualise, _display, benchmark):
    ...
    if benchmark:
        ...
        if VISUALISE:
            ...
            vis.init(point_cloud, DELTA_ALPHA, LIDAR_RANGE, BIN_SIZE, VISUALISE, FIGURES_DIR)
