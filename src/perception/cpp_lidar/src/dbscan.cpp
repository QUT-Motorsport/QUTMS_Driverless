...
vector<Point> DBSCAN::getClusteredPoints(int clusterID) {
    vector<Point> clusteredPoints;
    vector<Point>::iterator iter;
    for (iter = m_points.begin(); iter != m_points.end(); ++iter) {
        if (iter->clusterID == clusterID) {
            clusteredPoints.push_back(*iter);
        }
    }
    return clusteredPoints;
}

int DBSCAN::getNumClusters() {
    int maxClusterID = 0;
    vector<Point>::iterator iter;
    for (iter = m_points.begin(); iter != m_points.end(); ++iter) {
        if (iter->clusterID > maxClusterID) {
            maxClusterID = iter->clusterID;
        }
    }
    return maxClusterID;
}
