...
def get_discretised_positions(
    ...
    segments_idx: np.ndarray = np.arctan(point_cloud['y'] / point_cloud['x']) / DELTA_ALPHA
    ...
    bins_idx: np.ndarray = point_norms / BIN_SIZE
...
def get_prototype_points(
        seg_bin_nrm_z: np.ndarray, 
        SEGMENT_COUNT: int, 
        BIN_COUNT: int,
    ) -> List[List[List]]:
    ...
    split_bin_nrm_z: np.ndarray = np.split(seg_bin_nrm_z, np.where(np.diff(seg_bin_nrm_z[:, 0]))[0] + 1)

    # For every bin in every each segment a prototype point may be assigned
    prototype_points: List[List[List]] = [[[] for j in range(BIN_COUNT)] for i in range(SEGMENT_COUNT)]
    ...
    for i in range(len(split_bin_nrm_z)):
        ...
        split_split_nrm_z: np.ndarray = np.split(split_bin_nrm_z[i], np.where(np.diff(split_bin_nrm_z[i][:, 1]))[0] + 1)
        ...
        for j in range(len(split_split_nrm_z)):
            ...
            min_height_idx: np.ndarray = np.argsort(split_split_nrm_z[j][:, 3])[0]
