...
def generate_launch_description():
    ...
    config_common_path = os.path.join(get_package_share_directory("zed_driver"), "config", "common.yaml")

    config_camera_path = os.path.join(get_package_share_directory("zed_driver"), "config", "zed2i.yaml")
    ...
    zed_wrapper_launch = IncludeLaunchDescription(
        launch_description_source=PythonLaunchDescriptionSource(
            [get_package_share_directory("zed_wrapper"), "/launch/include/zed_camera.launch.py"]
        ),
        launch_arguments={
