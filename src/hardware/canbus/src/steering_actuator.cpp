...
void SteeringActuator::configure_c5e() {
    ...
    RCLCPP_INFO(get_logger(), "Configured C5E");
}

void SteeringActuator::c5e_state_request_callback() {
    this->read_steering_data(STATUS_WORD);
    // RCLCPP_INFO_THROTTLE(get_logger(), *this->get_clock(), 250, "Requesting state");
...
void SteeringActuator::steering_angle_callback(const std_msgs::msg::Float32::SharedPtr msg) {
    ...
    RCLCPP_INFO_ONCE(get_logger(), "Steering angle received");
    RCLCPP_DEBUG(get_logger(), "Current angle: %f", msg->data);
}
...
void SteeringActuator::steering_target_callback(const std_msgs::msg::Float32::SharedPtr msg) {
    ...
    // RCLCPP_INFO_THROTTLE(get_logger(), *this->get_clock(), 500, "Requested angle: %f",
...
void SteeringActuator::steer_can_callback(const driverless_msgs::msg::Can::SharedPtr msg) {
    ...
    if (msg->id == C5E_EMCY_ID) {
        ...
        RCLCPP_ERROR(get_logger(), "Emergency error code: %x", error_code);
    } else if (msg->id == C5E_BOOT_UP_ID) {
        RCLCPP_INFO(get_logger(), "C5E Booted Up");
        ...
        if (!initial_enc_saved_) {
            RCLCPP_INFO_ONCE(get_logger(), "Position received");
            // initial_enc_ = val;
        ...
        if (msg->data[0] == 0x80) {
            ...
            RCLCPP_INFO(get_logger(), "ACK object: %x, %x", object_id, msg->data[0]);
            return;
        }
        ...
        if (object_id == STATUS_WORD) {
            ...
            RCLCPP_DEBUG_THROTTLE(get_logger(), *get_clock(), 100, "Status word: %s",
                                  std::bitset<16>(status_word).to_string().c_str());
            RCLCPP_DEBUG_THROTTLE(get_logger(), *get_clock(), 100, "Current state: %s", current_state_.name.c_str());
            ...
            RCLCPP_DEBUG(get_logger(), "Desired state: %s", desired_state_.name.c_str());

            // State transition stage via state map definitions
            if (current_state_ != desired_state_) {
                RCLCPP_INFO(get_logger(), "Current state: %s", current_state_.name.c_str());
                RCLCPP_INFO(get_logger(), "Sending state: %s", desired_state_.name.c_str());
...
void SteeringActuator::pre_op_centering() {
    ...
    if (centre_stage_ == 0) {
        RCLCPP_INFO_ONCE(this->get_logger(), "Center->stage 0");
        current_velocity_ = this->get_parameter("velocity").as_int();
        current_acceleration_ = this->get_parameter("acceleration").as_int();
        ...
        RCLCPP_INFO_THROTTLE(this->get_logger(), *this->get_clock(), 1000, "Center->stage 1: %d/%d",
        ...
        RCLCPP_INFO_ONCE(this->get_logger(), "Center->stage 2");
        current_velocity_ = this->get_parameter("velocity_centering").as_int();
        current_acceleration_ = this->get_parameter("acceleration_centering").as_int();
        ...
        current_acceleration_ = this->get_parameter("acceleration").as_int();
        control_method_ = MODE_ABSOLUTE;
        RCLCPP_INFO_ONCE(this->get_logger(), "Centered->stage 3, offset: %d", offset_);
        this->configure_c5e();
        return;
    }
}
...
void SteeringActuator::send_steering_data(uint16_t obj_index, uint8_t *data, size_t data_size) {
    ...
    canmsg_callback(std::move(_d_2_f(id, 0, out, sizeof(out))));
...
SteeringActuator::SteeringActuator(const rclcpp::NodeOptions &options) : Node("canbus_translator_node", options) {
    // Steering parameters
    ...
    this->declare_parameter<float>("rough_centre_range", 10.0);
