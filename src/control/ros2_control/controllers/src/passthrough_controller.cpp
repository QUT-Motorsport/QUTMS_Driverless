...
/* on_init() function: This function is called when the controller is initialized.
It is used to initialize the controller's parameters and resources.*/

controller_interface::CallbackReturn PassthroughController::on_init() {
    try {
        param_listener_ = std::make_shared<ParamListener>(get_node());  // listen to parameters
        params_ = param_listener_->get_params();                        // get the parameters from the listener
...
// state_interface_configuration() function: This function is used to configure the state interfaces
// of the controller.
...
controller_interface::CallbackReturn PassthroughController::on_configure(
    ...
    joints_cmd_sub_ = this->get_node()->create_subscription<DataType>(
        "~/commands", rclcpp::SystemDefaultsQoS(), [this](const DataType::SharedPtr msg) {
            ...
            if (msg->data.size() == command_interface_names_.size()) {
                ...
                RCLCPP_ERROR(this->get_node()->get_logger(), "Invalid command received of %zu size, expected %zu size",
...
controller_interface::CallbackReturn PassthroughController::on_activate(
    ...
    if (!controller_interface::get_ordered_interfaces(command_interfaces_, command_interface_names_, std::string(""),
                                                      ordered_interfaces) ||
...
controller_interface::return_type PassthroughController::update_and_write_commands(const rclcpp::Time& /*time*/,
                                                                                   const rclcpp::Duration& /*period*/) {
    for (size_t i = 0; i < command_interfaces_.size(); ++i)  // for each command interface
    {
        if (!std::isnan(reference_interfaces_[i]))  // if reference_interface is not NaN
        {
            command_interfaces_[i].set_value(
                reference_interfaces_[i]);  // set the value of the command_interface to be reference_interface
        }
    }
...
std::vector<hardware_interface::CommandInterface> PassthroughController::on_export_reference_interfaces() {
    std::vector<hardware_interface::CommandInterface> reference_interfaces;  // vector of reference interfaces
