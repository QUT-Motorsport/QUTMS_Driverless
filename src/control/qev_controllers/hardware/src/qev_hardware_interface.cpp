#include "qev_controllers/qev_hardware_interface.hpp"
...
// pass in the hardware info (command, state interfaces and joints) in the qev_ros2_control.xacro
hardware_interface::CallbackReturn QevHardwareInterface::on_init(const hardware_interface::HardwareInfo& info) {
    /* For now, info_ is a member variable of the class QevHardwareInterface, storing:
        info_.joints = {
            {
    ...
    */
    ...
    if (info_.joints.size() != 2) {
        RCLCPP_ERROR(get_logger(),
                     "QevHardwareInterface::on_init() - Failed to initialize, "
...
hardware_interface::CallbackReturn QevHardwareInterface::on_configure(const rclcpp_lifecycle::State& previous_state) {
    RCLCPP_INFO(get_logger(), "Configuring ...please wait...");

    for (int i = 0; i < hw_start_sec_; i++) {
        RCLCPP_INFO(get_logger(), "%.1f seconds left... [ on_configure() ]", static_cast<float>(hw_start_sec_ - i));
...
hardware_interface::CallbackReturn QevHardwareInterface::on_cleanup(const rclcpp_lifecycle::State& previous_state) {
...
hardware_interface::CallbackReturn QevHardwareInterface::on_activate(const rclcpp_lifecycle::State& previous_state) {
    RCLCPP_INFO(get_logger(), "Activating ...please wait...");

    for (auto i = 0; i < hw_start_sec_; i++) {
        rclcpp::sleep_for(std::chrono::seconds(1));
        RCLCPP_INFO(get_logger(), "%.1f seconds left.. [ on_activate() ].", hw_start_sec_ - i);
    }
...
hardware_interface::CallbackReturn QevHardwareInterface::on_deactivate(
    ...
    for (auto i = 0; i < hw_stop_sec_; i++) {
        rclcpp::sleep_for(std::chrono::seconds(1));
        RCLCPP_INFO(get_logger(), "%.1f seconds left... [ on_deactivate() ]", hw_stop_sec_ - i);
    }
...
hardware_interface::return_type QevHardwareInterface::read(const rclcpp::Time& time, const rclcpp::Duration& period) {
    // BEGIN: This part here is for exemplary purposes - Please do not copy to your production code
...
#include "pluginlib/class_list_macros.hpp"
PLUGINLIB_EXPORT_CLASS(qev_controllers::QevHardwareInterface, hardware_interface::SystemInterface)