...
def generate_launch_description():
    ...
    driver_params_file = str(get_package_share_path("velodyne_driver") / "config" / "VLP32C-velodyne_driver_node-params.yaml")
    ...
    convert_params_file = os.path.join(get_package_share_path("velodyne_pointcloud") / "config" / "VLP32C-velodyne_convert_node-params.yaml")
    with open(convert_params_file, "r") as f:
        convert_params = yaml.safe_load(f)["velodyne_convert_node"]["ros__parameters"]
    convert_params["calibration"] = str(get_package_share_path("velodyne_pointcloud") / "params" / "VeloView-VLP-32C.yaml")
    ...
    laserscan_params_file = str(get_package_share_path("velodyne_laserscan") / "config" / "default-velodyne_laserscan_node-params.yaml")
