...
class ConeLocator(Node):
    ...
    def odom_callback(self, odom_msg: Odometry):
        ...
        for cone in self.track:
            ...
            if (
                ref_cone.location.x > 0
                and ref_cone.location.x < self.max_range
                and ref_cone.location.y > -9
                and ref_cone.location.y < 9
            ):
                ref_cones.append(ref_cone)

        # create message with cones
        detection_msg = ConeDetectionStamped(header=odom_msg.header, cones=ref_cones)
